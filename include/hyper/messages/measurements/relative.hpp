...
class RelativeMeasurement final
    : public VariableMeasurement<TVariable> {
 public:
  ...
  RelativeMeasurement(const Stamp& stamp, const Sensor& sensor, const Stamp& other_stamp, const Sensor& other_sensor, const TVariable& variable)
      : VariableMeasurement<TVariable>{stamp, sensor, variable},
        other_stamp_{other_stamp},
        other_sensor_{&other_sensor} {}

  /// Other stamp accessor.
  /// \return Other stamp.
  [[nodiscard]] auto otherStamp() const -> const Stamp& {
    return other_stamp_;
  }

  /// Other stamp modifier.
  /// \return Other stamp.
  auto otherStamp() -> Stamp& {
    return static_cast<Stamp&>(std::as_const(*this).otherStamp());
  }

  /// Other sensor accessor.
  /// \return Other sensor.
  [[nodiscard]] auto otherSensor() const -> const Sensor& {
